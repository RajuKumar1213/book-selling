...
export const AddressForm: React.FC<AddressFormProps> = ({
  ...
  const { user, addAddress, updateAddress, deleteAddress, checkAuth } =
    useAuth();
  ...
  // Explicitly fetch user data when component mounts - ONLY ONCE
  useEffect(() => {
    const fetchUserData = async () => {
      // Prevent infinite loop by tracking initialization
  ...
  }, []); // Remove user and checkAuth from dependencies to prevent infinite loop

  // Separate effect to handle user data changes for UI updates only
  useEffect(() => {
    if (user && hasInitialized.current) {
      if (user.address && user.address.length > 0) {
        setIsLoadingAddresses(false);
  ...
  }, [user]); // Only watch user changes, don't call checkAuth    // Handle UI updates once we have user data
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      ...
      user.address.length > 0 &&
      !selectedAddress &&
      (orderForm.fullAddress || orderForm.area)
    ) {
      const matchingAddress = user.address.find(
        (addr) =>
          addr.fullAddress === orderForm.fullAddress ||
          addr.city === orderForm.area
      );
              ...
              <div className="group">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full pl-10 pr-3 py-3 border-2 rounded-lg focus:ring-0 focus:border-green-400 transition-all duration-300 ${
              ...
              <div className="group">
                ...
                <div className="relative">
                  <select
                    ...
                    className={`w-full pl-10 pr-8 py-3 border-2 rounded-lg focus:ring-0 focus:border-green-400 transition-all duration-300 appearance-none ${
                      formErrors.city
                        ? "border-red-400 bg-red-50"
                        : "border-gray-200"
                    }`}
              ...
              <div className="group">
                ...
                {!formData.city && (
                  <p className="mt-1 text-xs text-gray-500">
                    💡 Select an area above to auto-fill PIN code, or enter
                    manually
                  </p>
                )}
              </div>
            ...
            <div className="group">
              ...
              <div className="relative">
                <textarea
                  ...
                  rows={3}
                  className={`w-full pl-10 pr-3 py-3 border-2 rounded-lg focus:ring-0 focus:border-green-400 transition-all duration-300 resize-none ${
