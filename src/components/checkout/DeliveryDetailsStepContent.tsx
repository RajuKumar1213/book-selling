...
const DeliveryDetailsStepContent: React.FC = () => {
  const { state, dispatch, updateOrderForm, goToNextStep, goToPreviousStep } =
    useCheckout();
  ...
  const handleDateSelect = (date: string) => {
    ...
    dispatch({
      type: "SET_MODAL",
      payload: { modal: "showCalendar", value: false },
    });
    // Show delivery type modal after date selection
    dispatch({
      type: "SET_MODAL",
      payload: { modal: "showDateModal", value: true },
    });
  };
  ...
  const handleDeliveryTypeSelect = (deliveryType: any) => {
    ...
    dispatch({
      type: "SET_MODAL",
      payload: { modal: "showDateModal", value: false },
    });
    // Show time slot modal after delivery type selection
    dispatch({
      type: "SET_MODAL",
      payload: { modal: "showTimeSlotModal", value: true },
    });
  };
  const handleTimeSlotSelect = (timeSlot: string) => {
    ...
    dispatch({
      type: "SET_MODAL",
      payload: { modal: "showTimeSlotModal", value: false },
    });
  };
    ...
    try {
      ...
      if (!user) {
        ...
        const personalDetailsErrors = validatePersonalDetails({
          ...
          phone: orderForm.mobileNumber
            ? orderForm.mobileNumber.replace(/^\+?91/, "")
            : "",
        });
      ...
      if (user && (orderForm.fullName.trim() || orderForm.email.trim())) {
        try {
          ...
          const personalDetailsResult = await updateUser(updateData);
          if (personalDetailsResult.success) {
            console.log("✅ Personal details saved successfully");
          } else {
            console.warn(
              "⚠️ Failed to save personal details:",
              personalDetailsResult.error || "Unknown error"
            );
          }
        } catch (error) {
      ...
      if (
        ...
        console.log(
          "ℹ️ Skipping address save - user not logged in or address incomplete"
        );
      }
  ...
  const handleShowCalendar = () => {
    dispatch({
      type: "SET_MODAL",
      payload: { modal: "showCalendar", value: true },
    });
  };
