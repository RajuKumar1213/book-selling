...
import { safeLocalStorageSetItem } from "@/utils/localStorage";
...
const CartReviewStepContent: React.FC = () => {
  ...
  const { state, goToPreviousStep, dispatch } = useCheckout();
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (
        e.key === "bakingo-selected-addons" ||
        e.key === "bakingo-addon-quantities"
      ) {
        loadAddOnsFromStorage();
      }
    };
  ...
  const getSelectedDeliveryType = () => {
    ...
    return deliveryTypes.find(
      (type) => type.id === state.orderForm.deliveryType
    );
  };
    ...
    try {
      ...
      const orderData = {
        ...
        customerInfo: {
          ...
          mobileNumber: state.orderForm.mobileNumber
            .replace(/\s+/g, "")
            .replace(/^\+91/, ""), // Clean mobile number
      ...
      const stored = safeLocalStorageSetItem(
        "pending-order",
        JSON.stringify(orderDataToStore)
      );
  ...
  const orderSummary = calculateOrderSummary(
    cart,
    state.orderForm.deliveryType || "standard"
  );
  if (cart.length === 0) {
    return (
  ...
  return (
    <div className="p-3 md:p-6">
      ...
      <div className="mb-4 md:mb-6">
        <h2 className="text-lg md:text-xl font-normal text-gray-900 mb-1 md:mb-2">
          Review Your Order
        </h2>
        <p className="text-sm md:text-base text-gray-600">
          Check your items and delivery details before proceeding to payment
        </p>
      </div>
                ...
                <div className="flex-1">
                  ...
                  {item.selectedAddOns && item.selectedAddOns.length > 0 && (
                    <div className="mt-2 p-2 bg-pink-50 rounded border border-pink-200">
                      <h4 className="text-xs font-normal text-pink-800 mb-1">
                        Included Add-ons:
                      </h4>
                  ...
                  {item.customization?.type === "photo-cake" && (
                    <div className="mt-2 p-2 bg-purple-50 rounded border border-purple-200">
                      <h4 className="text-xs font-normal text-purple-800 mb-2">
                      ...
                      <div className="flex items-start space-x-2">
                        {(item.customization.imageUrl ||
                          ...
                          <div className="relative w-12 h-12 rounded-lg overflow-hidden flex-shrink-0">
                            <Image
                              src={
                                item.customization.imageUrl ||
                                (item.customization.image instanceof File
                                  ? URL.createObjectURL(
                                      item.customization.image
                                    )
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-purple-600 mt-1">
                            ✓ Custom photo will be printed on cake
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg p-3 md:p-4 border border-pink-200">
              ...
              <div className="space-y-2 md:space-y-3">
                {selectedAddOns.map((addOn) => (
                  <div
                    key={addOn._id}
                    className="flex items-center justify-between p-2 md:p-3 bg-white rounded border border-pink-200"
                  >
                    <div className="flex items-center space-x-2 md:space-x-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-normal text-gray-800 text-xs md:text-sm">
                          {addOn.name}
                        </h4>
              ...
              <div className="flex justify-between">
                <span>
                  {getSelectedDeliveryType()
                    ? `${getSelectedDeliveryType()?.name} (${
                        getSelectedDeliveryType()?.description
                      })`
                    : "Delivery charges"}
                </span>
                <span>
            ...
            {selectedAddOns.length > 0 && (
              <div className="mt-4 p-3 bg-pink-50 border border-pink-200 rounded-lg">
                <p className="text-sm text-pink-700 font-medium">
                  ✨ {selectedAddOns.length} special add-on
                  {selectedAddOns.length > 1 ? "s" : ""} included!
                </p>
              </div>
            )}
          </div>
      ...
      {showPaymentLink && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg text-center">
          ...
          <p className="text-green-700 text-sm mb-3">
            Your order has been created. Please proceed to payment:
          </p>
          <button
            onClick={() => {
              dispatch({ type: "SET_STEP", payload: 3 });
