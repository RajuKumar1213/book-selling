...
const baseProductSchema = z.object({
  ...
  hasFlavours: z.boolean(),
  flavours: z
    .array(z.string().min(1, "Flavour name cannot be empty"))
    .optional(),
...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  // Determine if editing and has existing images
  const isEditing = !!product;
  const hasExistingImages = !!(
    product?.imageUrls && product.imageUrls.length > 0
  );

  // localStorage key for recent product data
  const RECENT_PRODUCT_KEY = 'recent_product_form_data';

  // Function to save form data to localStorage
  const saveToLocalStorage = (formData: any) => {
    try {
      const dataToSave = {
        ...
        hasFlavours: formData.hasFlavours,
        flavours: flavours,
        timestamp: Date.now()
      };
      localStorage.setItem(RECENT_PRODUCT_KEY, JSON.stringify(dataToSave));
    } catch (error) {
  ...
  const loadFromLocalStorage = () => {
    try {
      const savedData = localStorage.getItem(RECENT_PRODUCT_KEY);
      if (savedData) {
  ...
  useEffect(() => {
    if (product) {
      // EDITING MODE: Load actual product data
      ...
      // Handle flavours
      setValue("hasFlavours", product.hasFlavours || false);
      const productFlavours = product.flavours || [];
      setValue("flavours", productFlavours);
      setFlavours(productFlavours); 
      ...
      // NEW PRODUCT MODE: Load from localStorage if available
      const savedData = loadFromLocalStorage();
      if (savedData) {
        
        // Load saved data (but not name and images)
        setValue("description", savedData.description || "");
        setValue("price", savedData.price || 0);
        setValue("discountedPrice", savedData.discountedPrice || 0);
        setValue("categories", savedData.categories || []);
        setSelectedCategories(savedData.categories || []);
        
        // Load weight options
        if (savedData.weightOptions && savedData.weightOptions.length > 0) {
          setValue("weightOptions", savedData.weightOptions);
          setWeightOptions(savedData.weightOptions);
        }
        
        setValue("isAvailable", savedData.isAvailable !== undefined ? savedData.isAvailable : true);
        setValue("isBestseller", savedData.isBestseller || false);
        setValue("preparationTime", savedData.preparationTime || "");
        setValue("hasFlavours", savedData.hasFlavours || false);
        
        // Load flavours
        if (savedData.flavours && savedData.flavours.length > 0) {
          setValue("flavours", savedData.flavours);
          setFlavours(savedData.flavours);
        }
      }
    }
  }, [product, setValue]);

  // Keep form categories in sync with selectedCategories
  useEffect(() => {
  ...
  const onSubmit = async (data: ProductFormData) => {
    try {
      // Ensure categories are in sync
      const categoriesToSubmit =
        selectedCategories.length > 0 ? selectedCategories : data.categories;
      ...
      const safeData = {
        ...
        categories: Array.isArray(categoriesToSubmit)
          ? [...categoriesToSubmit]
          : [],
        ...
        hasFlavours: Boolean(data.hasFlavours),
        flavours: data.hasFlavours ? [...flavours] : [],
      ...
      try {
        ...
        cleanData = {
          ...
          hasFlavours: Boolean(safeData.hasFlavours),
          flavours: [...safeData.flavours],
      ...
      // Add flavour data
      formData.append("hasFlavours", cleanData.hasFlavours.toString());
      if (cleanData.hasFlavours && cleanData.flavours.length > 0) {
        cleanData.flavours.forEach((flavour: string) => {
          formData.append("flavours", flavour);
        });
      ...
      if (product) {
        ...
        if (response.data.success) {
          // Save current form data to localStorage for next time
          saveToLocalStorage(cleanData);
          
          showSuccess("Success!", "Product created successfully! Data saved for quick addition of similar products."); 
          
          // Reset only unique fields that should be different for each product
          setValue("name", ""); // Clear name for new product
          setImageFiles([]);
          setImagePreviews([]);
          setExistingImages([]);
          setRemovedImages([]);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] flex flex-col relative">
          ...
          {!product && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h3 className="text-sm font-medium text-blue-800">Smart Form Mode</h3>
                  <p className="text-sm text-blue-700 mt-1">
                    {loadFromLocalStorage() ? 
                      "Previous product data loaded to help you add similar products quickly. Only product name and images need to be added." :
                      "After adding your first product, the form will remember your data to speed up adding similar products."
                    }
                  </p>
                </div>
              </div>
                      ...
                      render={({ field }) => (
                        <label className="flex items-center space-x-3 cursor-pointer group">
                          ...
                          <span className="text-sm font-medium text-gray-700 group-hover:text-orange-600 transition-colors">
                            This product has multiple flavours
                          </span>
                        </label>
                      )}
                    />

                    {/* Flavours Input - Only show if hasFlavours is true */}
                    <Controller
                      name="hasFlavours"
                      control={control}
                      render={({ field }) => (
                          ...
                          {field.value && (
                            <div className="space-y-4">
                                ...
                                <input
                                  ...
                                  onKeyPress={(e) => {
                                    if (e.key === "Enter") {
                                      ...
                                      if (
                                        newFlavour.trim() &&
                                        !flavours.includes(newFlavour.trim())
                                      ) {
                                        const updatedFlavours = [
                                          ...flavours,
                                          newFlavour.trim(),
                                        ];
                                        setFlavours(updatedFlavours);
                                        setValue("flavours", updatedFlavours);
                                        setNewFlavour("");
                                      }
                                    }
                                  }}
                                  className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                                  placeholder="Enter flavour name (e.g., Chocolate, Vanilla, Strawberry)"
                                />
                                <button
                                  type="button"
                                  onClick={() => {
                                  ...
                                  className="px-4 py-2 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-lg hover:from-orange-600 hover:to-pink-600 transition-all duration-200 font-medium">
                                  Add
                                </button>
                              </div>

                              {/* Display added flavours */}
                              {flavours.length > 0 && (
                                <div className="flex flex-wrap gap-2">
                                  {flavours.map((flavour, index) => (
                                    <span
                                      key={index}
                                      className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gradient-to-r from-orange-100 to-pink-100 text-orange-800 border border-orange-200">
                                      {flavour}
                                      <button
                                        type="button"
                                        onClick={() => {
                              ...
                              <p className="text-sm text-gray-500">
                                Add flavours that customers can choose from for
                                this product. Press Enter or click Add to add
                                each flavour.
                              </p>
                            </div>
                          )}
                ...
                {selectedCategories.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <p className="text-xs text-blue-700 mt-2">
                      Click the X to remove categories. All selected categories
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-3 rounded-b-xl flex flex-row gap-3 justify-between">
          <div className="flex gap-3">
            ...
            <button
              ...
              onClick={() => {
                ...
                // Clear localStorage
                localStorage.removeItem(RECENT_PRODUCT_KEY);
                
                showSuccess("Cleared!", "Form and saved data cleared successfully.");
              }}
              className="px-4 py-2 border border-orange-300 text-orange-600 rounded-md hover:bg-orange-50 transition-colors text-sm font-medium">
              Clear All
            </button>
          </div>{" "}
          <button
