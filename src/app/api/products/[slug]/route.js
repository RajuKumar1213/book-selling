...
export async function PUT(request, { params }) {
  try {
    ...
    if (contentType && contentType.includes("multipart/form-data")) {
      ...
      // Get flavour data
      const hasFlavours = formData.get("hasFlavours");
      ...
      // Handle flavours
      if (hasFlavours !== null && hasFlavours !== undefined) {
        updateData.hasFlavours = hasFlavours === "true";
        updateData.flavours = updateData.hasFlavours
          ? flavours.filter((f) => f && f.trim() !== "")
          : [];
      }
      ...
      if (!name || !description || !price || !imageUrls || !categories) {
        return NextResponse.json(
          {
            error:
              "Name, description, price, imageUrls, and categories are required",
          },
          { status: 400 }
        );
      }
