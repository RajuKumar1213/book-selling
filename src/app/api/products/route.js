...
function checkCloudinaryConfig() {
  ...
  if (missingVars.length > 0) {
    return {
      error: `Cloudinary configuration incomplete. Missing: ${missingVars.join(
        ", "
...
export async function GET(request) {
  try {
    ...
    const {
      ...
      limit: limitNum,
      skip,
    } = calculatePagination(page, limit); // Create sort options
    ...
    if (filters._useAggregation || sort._categorySpecificOrder) {
      ...
      let pipeline = [...aggregationSteps, { $match: filters }]; // Handle category-specific ordering
                      ...
                      $arrayElemAt: [
                        {
                          $filter: {
                            ...
                            cond: {
                              $eq: ["$$this.category", categoryObjectId],
                            },
                          },
                        },
                        0,
    ...
    if (sortBy === "displayOrder" && categoryObjectId) {
      formattedProducts.forEach((product, index) => {
        const categoryOrder = product.categoryOrders?.find(
          (order) => order.category?.toString() === categoryObjectId.toString()
        );
      });
    }
...
export async function POST(request) {
  try {
    ...
    const discountedPrice =
      discountedPriceStr && parseFloat(discountedPriceStr) > 0
    ...
    const bestsellerOrder = formData.get("bestsellerOrder")
      ? parseInt(formData.get("bestsellerOrder"))
      : undefined;

    // Get flavour data
    const hasFlavours = formData.get("hasFlavours") === "true";
    const flavours = formData.getAll("flavours");
    ...
    while (formData.has(`weightOptions[${index}][weight]`)) {
      ...
      const price = priceStr ? parseFloat(priceStr) : 0;
      const discountedPrice = formData.get(
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Name, description, categories, and weight options are required",
        },
        { status: 400 }
      );
    }
    ...
    const productData = {
      ...
      hasFlavours: Boolean(hasFlavours),
      flavours: hasFlavours ? flavours.filter((f) => f && f.trim() !== "") : [],
    ...
    const categoryOrderPromises = categories.map(async (categoryId) => {
      const maxOrder = await Product.aggregate([
        ...
        { $match: { "categoryOrders.category": categoryId } },
        {
          $group: {
            _id: null,
            maxOrder: { $max: "$categoryOrders.displayOrder" },
          },
        },
      ]);

      const nextOrder =
        maxOrder.length > 0 ? (maxOrder[0].maxOrder || 0) + 1 : 1;
...
export async function PATCH(request) {
  try {
    ...
    while (formData.get(`weightOptions[${index}][weight]`)) {
      const weight = formData.get(`weightOptions[${index}][weight]`);
      const price =
        parseFloat(formData.get(`weightOptions[${index}][price]`)) || 0;
      const discountedPrice = formData.get(
        `weightOptions[${index}][discountedPrice]`
      );
