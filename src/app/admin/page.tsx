  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      try {
        ...
        const recentRevenue = recentOrders7Days.reduce(
          (sum: number, order: any) => sum + (order.totalAmount || 0),
          0
        );
        const previousRevenue = previousOrders7Days.reduce(
          (sum: number, order: any) => sum + (order.totalAmount || 0),
          0
        );
        ...
        const revenueChange =
          previousRevenue > 0
            ...
            : 0;
        const ordersChange =
          previousOrders7Days.length > 0
            ? ((recentOrders7Days.length - previousOrders7Days.length) /
                previousOrders7Days.length) *
              100
            : 0; // Get unique customers count
        const uniqueCustomers = new Set(
          orders
            .map(
              (order: any) =>
                order.customerInfo?.mobileNumber || order.customerInfo?.fullName
            )
          ...
          {statsLoading ? (
            ...
            <>
              ...
              <StatCard
                ...
                change={`Avg: ₹${
                  dashboardStats?.averageOrderValue?.toLocaleString("en-IN", {
