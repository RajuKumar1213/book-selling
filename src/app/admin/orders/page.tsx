    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 mb-4 text-xs sm:text-sm">
        ...
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <span>
            {formatDate(order.estimatedDeliveryDate)} at {order.timeSlot}
          </span>
        </div>
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <span>
            ...
            {order.addons && order.addons.length > 0 && (
              <span>
                {" "}
                + {order.addons.length} add-on
                {order.addons.length > 1 ? "s" : ""}
              </span>
            )}
          </span>
        </div>
      ...
      {/* Photo Cake Display with Image Preview */}
      {order.items.some(
        (item) => item.customization?.type === "photo-cake"
      ) && (
            ...
            .map((item, index) => (
              <div
                ...
                item.customization.imageUrl !== "null" ? (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (
                        ...
                        alert(
                          "❌ Image URL is invalid or missing. Please check if the image was uploaded correctly."
                        );
                      }
                    }}
                  ...
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-lg flex-shrink-0 border-2 border-red-300 bg-red-50 flex items-center justify-center">
                    <div className="text-center">
                      <span className="text-red-600 text-xs font-bold block">
                        NO
                      </span>
                ...
                <div className="flex-1 min-w-0">
                  {" "}
                  <p className="text-purple-800 font-medium text-xs sm:text-sm truncate">
                    {item.name}
                  </p>
                  {item.customization?.message && (
                    <p
                      className="text-purple-700 text-xs italic truncate"
                      title={item.customization.message}>
                  ...
                  {item.customization?.imageUrl && (
                    <p
                      ...
                      title="Raw image URL for debugging">
                      URL:{" "}
                      {item.customization.imageUrl.length > 50
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
        <select
          value={order.status}
          onChange={(e) =>
            onStatusChange(order.orderId, e.target.value as Order["status"])
          }
          disabled={isUpdating}
        ...
        <button
          ...
          disabled={isUpdating}
          className="bg-red-600 text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium hover:bg-red-700 transition-colors flex items-center justify-center gap-1 sm:gap-2 disabled:opacity-50">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <button
                  onClick={() => {
                    ...
                    onClose(); // Close modal after delete action starts
                  }}
            ...
            <div className="bg-green-50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <p>
                  ...
                  <span className="font-medium">{order.customerInfo.area}</span>
                </p>
                <p>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs sm:text-sm text-gray-600">
                      ...
                      {item.selectedFlavour && (
                        <span>Flavour: {item.selectedFlavour}</span>
                      )}
                    ...
                    {item.customization?.type === "photo-cake" && (
                      <div className="mt-2 p-2 bg-purple-50 rounded border border-purple-200">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            {item.customization.message && (
                              <p className="text-purple-700 text-xs italic mb-1">
                                � Name: "{item.customization.message}"
                              </p>
                            )}{" "}
                            <p className="text-purple-600 text-xs font-medium">
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {order.addons.map((addon, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg bg-pink-50">
                    {addon.image ? (
                      <img
                        src={addon.image}
                        alt={addon.name}
...
export default function AdminOrders() {
  ...
  const fetchOrders = async (page = currentPage, showLoader = true) => {
    try {
      ...
      const response = await fetch(
        `/api/orders?limit=${itemsPerPage}&page=${page}`
      );
      const data = await response.json();
      if (data.success) {
        // Debug: Log photo cake items to see their structure
        data.orders.forEach((order: any) => {
          const photoCakeItems = order.items.filter(
            (item: any) => item.customization?.type === "photo-cake"
          );
          if (photoCakeItems.length > 0) {
            console.log(
              `🔍 Order ${order.orderId} has ${photoCakeItems.length} photo cake(s):`,
        ...
        if (data.pagination) {
          ...
          setTotalPages(
            data.totalPages ||
              Math.ceil((data.total || data.orders.length) / itemsPerPage)
          );
          setCurrentPage(page);
        }
  ...
  const handleDelete = async (orderId: string) => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete order #${orderId}?\n\nThis action cannot be undone and will permanently remove the order from the database.`
    );
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh orders to update stats and pagination
        fetchOrders(currentPage, false);
      } else {
        showError(
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">
            Fetching latest order data from database
          </p>
        </div>
      </div>
    );
